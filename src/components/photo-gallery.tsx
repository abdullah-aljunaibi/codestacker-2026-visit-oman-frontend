...
export function PhotoGallery({ images, name, slug, photosLabel = "Photos" }: PhotoGalleryProps) {
  ...
  return (
    <>
      ...
      {lightboxOpen && (
        <div className="photoGalleryLightbox" onClick={() => setLightboxOpen(false)}>
          <button className="photoGalleryClose" onClick={() => setLightboxOpen(false)} aria-label="Close">✕</button>
          ...
          <div className="photoGalleryCounter">{selectedIndex + 1} / {images.length}</div>
        </div>
      )}
    </>
  );
